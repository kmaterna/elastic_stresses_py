	...
	for line in ifile:
		...
		if temp[0]=='S:':
			if " SS " in line or " N " in line or " R " in line or " ALL " in line:  # reading wells and coppersmith convenient format
				[strike,rake,dip,magnitude,faulting_type,fault_lon,fault_lat,fault_depth]=read_source_line_WCconvention(line);
				[xstart, xfinish, ystart, yfinish, Kode, rtlat, reverse, top, bottom, comment] = compute_params_for_WC_source(strike, dip, rake, fault_depth, magnitude, faulting_type, fault_lon, fault_lat, zerolon, zerolat);				
				...
				[xstart, xfinish, ystart, yfinish, Kode, rtlat, reverse, top, bottom, comment] = compute_params_for_slip_source(strike, dip, rake, fault_depth, L, W, fault_lon, fault_lat, slip, zerolon, zerolat);
			one_source_object = coulomb_collections.Faults_object(xstart=xstart, xfinish=xfinish, ystart=ystart, yfinish=yfinish, Kode=Kode, rtlat=rtlat, reverse=reverse, potency=[], strike=strike, dipangle=dip, rake=rake, top=top, bottom=bottom, comment=comment);
			...
			[xstart, xfinish, ystart, yfinish, Kode, rtlat, reverse, top, bottom, comment] = compute_params_for_slip_source(strike, dip, rake, fault_depth, L, W, fault_lon, fault_lat, 0, zerolon, zerolat);
...
def compute_grid_parameters(minlon, maxlon, zerolon, minlat, maxlat, zerolat):	
	...
	start_gridx=0; start_gridy=0; finish_gridx=0; finish_gridy=0;
...
def compute_params_for_WC_source(strike, dip, rake, depth, magnitude, faulting_type, fault_lon, fault_lat, zerolon, zerolat):
	...
	xstart,ystart=conversion_math.add_vector_to_point(xcenter,ycenter,0,strike); # if the hypocenter is on one side of the rupture
