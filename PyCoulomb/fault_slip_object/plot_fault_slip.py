...
def map_source_slip_distribution(fault_dict_list, outfile, disp_points=None,
    ...
    if disp_x is not None:
        ...
        scale = (np.max(np.abs(disp_y)) * (1/scale_arrow[0]) / 0.012);  # empirical scaling to get convenient display
        ...
        fig.plot(x=[region[0]+0.30], y=[region[2]+0.05],  style='v0.2c+e+gblack+h0+p1p,black+z'+str(scale),
                 direction=[[scale_arrow[0]], [0]],  pen="thin,black");  # scale vector
        fig.text(x=[region[0]+0.45], y=[region[2]+0.15], text=scale_arrow[1]+' model');  # scale label
